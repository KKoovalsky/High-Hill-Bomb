...
void arm_countdown() {
	...
	if(to_boom_cnt <= half_to_boom_time >> 1) { BUZZER1_SW; BUZZER2_SW; return; }
...
void get_root_code() {
	...
	if(keys_pressed_num == KEY_NUM) {
		...
		if(eeprom_read_word(&changeable_vars[ADMIN_CODE]) == (uint16_t)atoi((char*)buffer)) {
			wait_flag = false;
...
void change_ee_var() {
	...
	if(key_pressed == '*' || key_pressed == '#') {
		if((ee_var_ptr != &changeable_vars[ARM_CODE]) || (ee_var_ptr != &changeable_vars[ADMIN_CODE])) {
			update_ee();
			main_exec = NULL;
			return;
		} else return;
